...
export class RestrictionsService {
  ...
  async create(data: Partial<BookingRestriction>): Promise<BookingRestriction> {
    const existing = await this.restrictionsRepo.findOne({ where: { name: data.name } });
    if (existing) throw new ConflictException('Restriction with this name already exists');
  ...
  async update(id: string, attrs: Partial<BookingRestriction>): Promise<BookingRestriction> {
    ...
    if (attrs.name && attrs.name !== entity.name) {
      const existing = await this.restrictionsRepo.findOne({ where: { name: attrs.name } });
      if (existing) throw new ConflictException('Restriction with this name already exists');
    }
