...
function sanitizeEmailHtml(raw: string): string {
  const withPlaceholder = raw.replace(/href="{{cancelUrl}}"/g, `href="${CANCEL_PLACEHOLDER}"`);
  const sanitized = sanitizeHtml(withPlaceholder, ALLOWED_HTML);
  return sanitized.replace(new RegExp(`href="${CANCEL_PLACEHOLDER}"`, 'g'), 'href="{{cancelUrl}}"');
}
