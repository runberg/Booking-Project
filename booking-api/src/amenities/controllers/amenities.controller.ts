...
export class AmenitiesController {
  ...
  async listAvailability() {
    ...
    const result: Array<{ id: string; name: string; status: 'free' | 'booked' | 'closed'; freeUntil?: string | null; nextAvailable?: string | null; availableForDays?: number | null }> = [];

    for (const a of amenities) {
      const open = parseTime(a.openTime || '09:00');
      const close = parseTime(a.closeTime || '22:00');
      const slotLength = a.slotLength || 60;
      const daysAhead = idToRestriction.get(a.bookingRestrictionId || '')?.daysAhead ?? 14;

      // Helper to fetch booked start times for a date
      const getBookedSetForDate = async (dateKey: string) => {
        const list = await this.bookingsRepo.find({ where: { amenityId: a.id, date: dateKey } });
      ...
      if (nowMinutes < open || nowMinutes >= close) {
        ...
        let searchStartDate = nowMinutes < open ? new Date(now) : addDays(now, 1);
        ...
        for (let d = 0; d <= daysAhead; d++) {
          ...
          for (let t = open; t < close; t += slotLength) {
            const key = toHHmm(t);
            if (!booked.has(key)) { found = { date: dateKey, time: key }; break; }
          }
          if (found) break;
        }
        result.push({ id: a.id, name: a.name, status: 'closed', nextAvailable: found ? `${found.date} ${found.time}` : null, freeUntil: null, availableForDays: null });
        continue;
      }

      // Open now. Build today's slots and booking set
      const bookedToday = await getBookedSetForDate(todayKey);
      const currentSlotStart = open + Math.floor(Math.max(0, nowMinutes - open) / slotLength) * slotLength;
      ...
      const findNextBooked = async (): Promise<{ date: string; time: string } | null> => {
        // Today from currentSlotStart onwards
        for (let t = Math.max(currentSlotStart, open); t < close; t += slotLength) {
          const key = toHHmm(t);
      ...
      const findNextFree = async (): Promise<{ date: string; time: string } | null> => {
      ...
      if (isCurrentBooked) {
        ...
        result.push({ id: a.id, name: a.name, status: 'booked', nextAvailable: nf ? `${nf.date} ${nf.time}` : null, freeUntil: null, availableForDays: null });
      } else {
        ...
        if (nextBooked) {
          ...
          const minutesUntil = nextBooked.date === todayKey ? nbMinutes - nowMinutes : Number.MAX_SAFE_INTEGER;
          if (minutesUntil < 20) {
            const nf = await findNextFree();
            result.push({ id: a.id, name: a.name, status: 'booked', nextAvailable: nf ? `${nf.date} ${nf.time}` : null, freeUntil: null, availableForDays: null });
          } else {
            result.push({ id: a.id, name: a.name, status: 'free', freeUntil: `${nextBooked.date} ${nextBooked.time}`, nextAvailable: null, availableForDays: null });
          }
        } else {
          // No bookings within the window: show available for next X days (limit daysAhead)
          result.push({ id: a.id, name: a.name, status: 'free', freeUntil: null, nextAvailable: null, availableForDays: daysAhead });
        }
      }
    }
